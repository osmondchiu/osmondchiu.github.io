---
layout: page
title: Policy work
excerpt: "A list of recent publications and previous events"

---

<h1>Policy Work</h1>

<h2>Overview</h2>
<p>
I work on public sector and labour market policy, with a focus on strengthening public 
institutions, improving workforce capability, and advancing social democratic reform.
</p>

<p>
My work combines policy analysis, research, and advocacy, drawing on experience across 
the public service, union movement, and think tank sector.
</p>

<h2>Key Areas of Work</h2>

<h3>Public Sector Reform</h3>
<ul>
  <li>Size, structure, and capability of the public service</li>
  <li>Outsourcing, privatisation, and service delivery models</li>
  <li>Governance, accountability, and institutional performance</li>
</ul>

<h3>Workforce Policy</h3>
<ul>
  <li>Employment conditions and job security</li>
  <li>Workforce planning and capability development</li>
  <li>The future of work, including automation and digital transformation</li>
</ul>

<h3>Social and Economic Policy</h3>
<ul>
  <li>Inequality, wages, and living standards</li>
  <li>Community wealth building and progressive economic policy</li>
  <li>Public policy responses to economic change</li>
</ul>

<h3>Multiculturalism and Representation</h3>
<ul>
  <li>Diversity in political and public institutions</li>
  <li>Representation of CALD communities</li>
  <li>Race, identity, and public discourse</li>
</ul>

<h2>Policy Skills</h2>

<h3>Policy Analysis and Development</h3>
<ul>
  <li>Policy design, evaluation, and reform development</li>
  <li>Legislative and regulatory analysis</li>
  <li>Comparative and international policy analysis</li>
</ul>

<h3>Quantitative and Data Analysis</h3>
<ul>
  <li>Analysis of ABS, OECD and administrative datasets</li>
  <li>Workforce modelling and labour market analysis</li>
  <li>Excel and R-based data analysis and visualisation</li>
</ul>

<h3>Research and Evidence Synthesis</h3>
<ul>
  <li>Literature reviews and evidence synthesis</li>
  <li>Translation of complex data into policy-relevant insights</li>
  <li>Preparation of research reports and briefing materials</li>
</ul>

<h3>Stakeholder Engagement and Advocacy</h3>
<ul>
  <li>Consultation with unions, government, and community organisations</li>
  <li>Preparation of submissions to parliamentary and policy inquiries</li>
  <li>Strategic policy advocacy and campaign support</li>
</ul>

<h3>Communication and Public Engagement</h3>
<ul>
  <li>Policy writing for public, media, and institutional audiences</li>
  <li>Media engagement and public commentary</li>
  <li>Briefings for decision-makers and stakeholders</li>
</ul>

<h2>Selected Policy Contributions</h2>

<ul>
  <li>
    <b>Public sector workforce analysis</b><br>
    Analysis of public sector size and composition, including contributions to debates 
    on workforce capability and service delivery.
  </li>

  <li>
    <b>Outsourcing and service delivery reform</b><br>
    Research and advocacy on the impacts of outsourcing and the case for rebuilding 
    in-house public sector capability.
  </li>

  <li>
    <b>Workforce reform and conditions</b><br>
    Policy work on employment conditions, job security, and workforce sustainability 
    across the public sector.
  </li>

  <li>
    <b>Multicultural representation</b><br>
    Analysis and advocacy on representation in political institutions and public debate.
  </li>
</ul>

<h2>Approach</h2>
<p>
My work is grounded in evidence-based analysis and informed by engagement with workers, 
institutions, and communities.
</p>

<ul>
  <li>Use of official data (ABS, OECD, administrative datasets)</li>
  <li>Policy modelling and comparative analysis</li>
  <li>Stakeholder engagement and applied research</li>
</ul>

<h2>Current Focus</h2>
<p>
Current areas of focus include public sector workforce capability, the future of work, 
and the role of public institutions in delivering equitable economic outcomes.
</p>

<h2>Contact</h2>
<p>
For policy collaboration, research engagement, or briefing requests, please get in touch 
via the Contact page.
</p>
